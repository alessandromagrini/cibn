...
In this example, we create simple Bayesian network for inferring risk attitude of bank customers.
...
The hypothesized Directed Acyclic Graph (DAG) is shown in the figure below. Also, we hypothesize a causal interaction between 'Marital' and 'Parent' in determining 'Risk'.

Here is the model code with prior parameter values. Each probability distribution is automatically normalized, thus the user can specify them using counts.
```
bankrisk_code <- '
